...
const FilaCostoSchema = z.object({
  ...
  tipoMoneda: z.string(),
  tipoCambio: z.number().min(0).optional().nullable(),
  cantidad: z.number().min(0),
  totalNeto: z.number(),
  iva: z.number().min(0),
  total: z.number(),
...
const ActaInputSchema = z.object({
  ...
  representanteEmail: z.string().optional().transform(v => (v && /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v) ? v : "")),
  representanteFono: z.string().optional(),
  contactoTecnico: z.string().optional(),
  contactoTecnicoEmail: z.string().optional().transform(v => (v && /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v) ? v : "")),
  contactoTecnicoFono: z.string().optional(),
  contactoFacturacion: z.string().optional(),
  contactoFacturacionEmail: z.string().optional().transform(v => (v && /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v) ? v : "")),
