...
export interface ActaData {
  id?: number;
  sres: string;           // Combobox: empresa destinataria (de BD)
  noActa: string;
  atencion: string;       // Combobox: persona de atención (de BD)
  activacionNueva: string;  // Texto plano: descripción de activación nueva
  fecha: string;
  razonSocial: string;
  ...
  firmaImagen?: string;  // base64 data URL de la firma dibujada o importada
  updatedAt?: string;
}
...
export function calcularResultado(ep: EPData): ResultadoCalculado {
  ...
  // Usamos el operador || para poner arreglos vacíos [] si la propiedad no existe
  const otrosGastos = ep.otrosGastos || [];
  const hardware = ep.hardware || [];
  const materiales = ep.materiales || [];
  const rrhh = ep.rrhh || [];
  const montoProyecto = ep.montoProyecto || 0;

  // Calcular totales por mes para Otros gastos
  const otrosMes1 = otrosGastos.filter(o => o.mes === 1).reduce((sum, o) => sum + o.total, 0);
  const otrosMes2 = otrosGastos.filter(o => o.mes === 2).reduce((sum, o) => sum + o.total, 0);
  const otrosMes3 = otrosGastos.filter(o => o.mes === 3).reduce((sum, o) => sum + o.total, 0);

  // Hardware y Materiales van al mes 1 por defecto
  const totalHardware = hardware.reduce((sum, h) => sum + h.total, 0);
  const totalMateriales = materiales.reduce((sum, m) => sum + m.total, 0);
  const totalRRHH = rrhh.reduce((sum, r) => sum + r.total, 0);
...
export function createDefaultEP(): EPData {
  return {
    ...
    otrosGastos: [
      ...
      // Mes 2
      createDefaultFilaOtros("comision", "Comisión", 2),
      createDefaultFilaOtros("movilizacion", "Movilización", 2),
      createDefaultFilaOtros("viatico", "Viático", 2),
      createDefaultFilaOtros("movilizacion", "Movilización", 2),
      createDefaultFilaOtros("viatico", "Viático", 2),
      createDefaultFilaOtros("movilizacion", "Movilización", 2),
      createDefaultFilaOtros("alojamiento", "Alojamiento", 2),
      createDefaultFilaOtros("varios", "Varios", 2),
      // Mes 3
      createDefaultFilaOtros("comision", "Comisión", 3),
      createDefaultFilaOtros("movilizacion", "Movilización", 3),
      createDefaultFilaOtros("viatico", "Viático", 3),
      createDefaultFilaOtros("movilizacion", "Movilización", 3),
      createDefaultFilaOtros("viatico", "Viático", 3),
      createDefaultFilaOtros("movilizacion", "Movilización", 3),
      createDefaultFilaOtros("alojamiento", "Alojamiento", 3),
      createDefaultFilaOtros("varios", "Varios", 3),
    ],
    status: "borrador",
  };
}
...
export interface Expediente {
  id: string;           // nanoid único
  nombre: string;       // "Expediente #1" o personalizado
  acta?: ActaData;      // F1 (puede estar incompleto)
  ep?: EPData;          // F2 (puede estar incompleto)
...
const STORAGE_KEY_EXPEDIENTES = "gestion_expedientes";
const STORAGE_KEY_EXPEDIENTE_ACTIVO = "gestion_expediente_activo";
...
// ─── Expediente storage helpers ───────────────────────────────────────────────

export function loadExpedientes(): Expediente[] {
  try {
    const raw = localStorage.getItem(STORAGE_KEY_EXPEDIENTES);
    return raw ? JSON.parse(raw) : [];
  } catch {
    return [];
  }
...
export function saveExpedientes(list: Expediente[]): void {
  try {
    localStorage.setItem(STORAGE_KEY_EXPEDIENTES, JSON.stringify(list));
  } catch (e) {
    console.warn("[Storage] Error saving expedientes:", e);
  }
}

export function loadExpedienteActivo(): string | null {
  return localStorage.getItem(STORAGE_KEY_EXPEDIENTE_ACTIVO);
}

export function saveExpedienteActivo(id: string | null): void {
  if (id) {
    localStorage.setItem(STORAGE_KEY_EXPEDIENTE_ACTIVO, id);
  } else {
    localStorage.removeItem(STORAGE_KEY_EXPEDIENTE_ACTIVO);
  }
}

export function crearExpediente(nombre?: string): Expediente {
  ...
  list.unshift(exp);
  saveExpedientes(list);
  saveExpedienteActivo(exp.id);
  return exp;
}

export function actualizarExpediente(id: string, updates: Partial<Omit<Expediente, "id" | "creadoEn">>): void {
  const list = loadExpedientes();
  const idx = list.findIndex(e => e.id === id);
  if (idx >= 0) {
    list[idx] = { ...list[idx], ...updates, actualizadoEn: new Date().toISOString() };
    saveExpedientes(list);
  }
}

export function getExpedienteById(id: string): Expediente | undefined {
  return loadExpedientes().find(e => e.id === id);
}

// ─── Hook: useExpediente ──────────────────────────────────────────────────────

export function useExpediente() {
  const [expedientes, setExpedientes] = useState<Expediente[]>(() => loadExpedientes());
  const [activoId, setActivoId] = useState<string | null>(() => loadExpedienteActivo());

  const expedienteActivo = expedientes.find(e => e.id === activoId) ?? null;

  const crear = useCallback((nombre?: string) => {
    const exp = crearExpediente(nombre);
  ...
  const renombrar = useCallback((id: string, nombre: string) => {
    actualizarExpediente(id, { nombre });
    setExpedientes(loadExpedientes());
  }, []);

  const guardarActa = useCallback((id: string, acta: ActaData) => {
    actualizarExpediente(id, { acta });
    setExpedientes(loadExpedientes());
  }, []);

  const guardarEP = useCallback((id: string, ep: EPData) => {
    actualizarExpediente(id, { ep });
    setExpedientes(loadExpedientes());
  }, []);

  const activar = useCallback((id: string) => {
    saveExpedienteActivo(id);
    setActivoId(id);
  }, []);

  const eliminar = useCallback((id: string) => {
    const list = loadExpedientes().filter(e => e.id !== id);
    saveExpedientes(list);
    setExpedientes(list);
    if (activoId === id) {
      const next = list[0]?.id ?? null;
      saveExpedienteActivo(next);
      setActivoId(next);
    }
  }, [activoId]);
